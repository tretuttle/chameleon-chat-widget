  ...
  switch (action.type) {
    ...
    case 'SUBMIT_INPUT': {
      ...
      if (activeFlow && currentStepId && activeFlow[currentStepId]?.allowTextInput) {
        ...
        if (!nextStep) {
          logger.error(`Next step "${nextStepId}" not found in flow.`);
          return state;
        }
    ...
    case 'PROCESS_BOT_RESPONSE': {
      ...
      if (crossFlowMap[nextStepId]) {
        ...
        if (!nextStep) {
          logger.error(`Step "${nextStepId}" not found in flow: ${newFlowType}`);
      ...
      if (!nextStep) {
        logger.error(`Step "${nextStepId}" could not be found in any known flow.`);
        return { ...state, isTyping: false };
      }
    ...
    case 'PROCESS_SUGGESTION_RESPONSE': {
      ...
      if (!startStep) {
        logger.error(`Starting step "${startStepId}" not found in flow: ${flowType}`);
        return { ...state, isTyping: false };
      }
