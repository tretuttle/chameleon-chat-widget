			...
			>
				...
				>
					{message.sender === "agent" ? (
						...
						<MessageCircle
							className={`text-white ${isCondensed ? "w-3 h-3" : "w-5 h-5"}`}
						/>
					)}
				</div>
				<div
					className={`px-6 py-4 rounded-2xl whitespace-pre-wrap shadow-sm leading-relaxed ${
						message.sender === "user"
	...
	return (
		<div className={`space-y-6 ${className}`}>
			{filteredMessages.map((message) =>
				renderMessage(message, !!condensed),
			)}
