...
const ModalChat = ({
	...
	const hasOnlyButtonOptions =
		currentStep &&
		currentStep.userOptions &&
		currentStep.userOptions.length > 0;
				...
				<div className="flex-1 overflow-y-auto px-8 py-6 space-y-6 min-h-[500px] bg-gradient-to-b from-blue-50 via-blue-25 to-white">
					...
					<div className="text-center pb-4 border-b border-blue-200">
						<h2 className="text-2xl font-semibold text-gray-800 mb-2">
							How can <span className="text-blue-500">Amigo</span>{" "}
							help?
						</h2>
						<p className="text-black text-sm font-medium">
							Amigo Support joined •{" "}
					...
					{conversationHistory
						...
						.map((message) => (
							...
							>
								<div
									className={`flex items-start space-x-4 max-w-[75%] ${message.sender === "user" ? "flex-row-reverse space-x-reverse" : ""}`}
								>
									<div
									...
									<div
										className={`px-6 py-4 rounded-2xl text-sm whitespace-pre-wrap shadow-sm leading-relaxed ${
											message.sender === "user"
